...
def add_event(cal, SUMMARY, DTSTART, DTEND, DESCRIPTION, LOCATION):
    ...
    create_time = datetime.datetime.today().strftime(r"%Y%m%dT%H%M%SZ")
    #create_time = time_format.format(date=DTSTAMP)
    cal.add_event(
        SUMMARY=SUMMARY,
...
def standardlized(classnum, r):
    calendar = Calendar(calendar_name=classnum)
    for a in r:
        subject1 = a[1] + '-' + a[3]
        time = a[4]
        yy = r'[0-9]*年'
        ...
        ecm = ec[1]
        #loc = r[7]
        init(calendar, e=subject1, loc=a[6], subject=subject1, y=int(y), m=int(m), d=int(
            d), beginhour=int(bch), endhour=int(ech), beginmin=int(bcm), endmin=int(ecm))
    calendar.save_as_ics_file()


def init(calendar, e, subject, loc, y, m, d, beginhour, endhour, beginmin, endmin):
    #print(y, m, d)
    add_event(calendar,
              SUMMARY=subject,
              DTSTART=datetime.datetime(
                  year=y, month=m, day=d, hour=beginhour, minute=beginmin, second=00),
              DTEND=datetime.datetime(
                  year=y, month=m, day=d, hour=endhour, minute=endmin, second=00),
              DESCRIPTION=e,
              LOCATION=loc)
...
if __name__ == '__main__':
    ...
    add_event(calendar,
              SUMMARY="测试",
              DTSTART=datetime.datetime(
                  year=2019, month=2, day=19, hour=21, minute=21, second=00),
              DTEND=datetime.datetime(
                  year=2019, month=2, day=19, hour=21, minute=30, second=00),
              DESCRIPTION="测试事件",
              LOCATION="我也不知道在哪儿")
    print(calendar.get_ics_text())
    calendar.save_as_ics_file()
